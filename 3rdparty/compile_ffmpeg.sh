...
# Configures and builds ffmpeg for the kval project.
# Usage:
#    ./compile_ffmpeg.sh <pin>, where pin is either pi1 or pi2.

SYSROOT="/mnt/mxq-rootfs"
...
if [ "$1" == "pi1" ]; then
	...
	ELDFLAGS="$SSH"
        ...
        export PKG_CONFIG_PATH="$MXQ_SYSROOT/usr/lib/aarch64-linux-gnu/pkgconfig"
        FFMPEG_ARCH=arm64
        SSH="-Wl,-rpath,$MXQ_SYSROOT/usr/lib/aarch64-linux-gnu"
        ECFLAGS="-mtune=cortex-a53 -funsafe-math-optimizations -lm $SSH"
        ELDFLAGS="$SSH"
elif [ "$1" == "x86" ]; then
        FFMPEG_ARCH=x86_64

else
        echo "Please choose either pi1, pi2, pi3 or s905x"
	exit 1
fi
...
# Check env.
if [ "$1" == "pi1" ] || [ "$1" == "pi1" ] || [ "$1" == "pi3" ]; then
    TEST="${SYSROOT:=false}"
elif [ "$1" == "s905x" ]; then
    TEST="${SYSROOT:=false}"
elif [ "$1" == "x86" ]; then
    TEST="${COMPILER_PATH:=true}"
else
    TEST="false"
fi

if [ "$TEST" == "false" ]; then
   echo "Please set SYSROOT."
   exit
fi
...
if [ "$1" == "pi1" ] || [ "$1" == "pi1" ] || [ "$1" == "pi3" ]; then
    check_exists_dir "$SYSROOT"
elif [ "$1" == "s905x" ]; then
    check_exists_dir "$SYSROOT"
elif [ "$1" == "x86" ]; then
    echo "Use system platform"
else
...
--enable-decoder=vp9 \
--enable-decoder=opus \
--cross-prefix=$GCC_PATH/gcc-linaro-6.1.1-2016.08-x86_64_aarch64-linux-gnu/bin/aarch64-linux-gnu- \
...
./configure \
--enable-rpath \
--extra-ldflags=-L$PWD/ffmpeg_compiled/lib \
--extra-cflags=-I$PWD/ffmpeg_compiled/include \
--enable-shared \
--enable-static \
--target-os=linux \
--enable-gpl \
