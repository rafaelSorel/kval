...
Rectangle {
    ...
    VideoOutput2 {
        ...
        SubtitleItem {
            ...
            rotation: -videooutput.orientation
            source: subtitle
            anchors.fill: parent
        }
        ...
        Text {
            ...
            opacity: 1
            anchors.fill: parent
            anchors.bottomMargin: Utils.scaled(100)
        }
    }
    ...
    UI_Subs {
        id: subtitle
        player: mediaPlayer.avplayer
        enabled: true
        autoLoad: true
        engines: ["FFmpeg", "LibASS"]
        delay: 0
//        fontFile: kvalUiConfigManager.qmlGetPath(KvalUiConfigManager.DEFAULT_TEXT_FONT)
//        fontFileForced: true
//        fontsDir: PlayerConfig.assFontsDir
    ...
    KvalUiSettingsManager {
        id: kvalUiSettingsManager
        onDynMenuReply: {
            settingsMainView.sViewDynaMenuReply(key, list)
        }
        onValueUpdated: {
            settingsMainView.sViewValueUpdated(grp, key, val)
        }
        onDisplayProgress: {
            pyNotificationView.requestProgressDisplay(
                kvalUiConfigManager.retranslate + qsTr("Settings Notification"),
                msg, val);
        }
        onDisplayMsg: {
            pyNotificationView.requestAlertDisplay(head, null, body);
        }

        onYesNoDiag: popUpView.yesNoDiag(title, body, no, yes, settingsMainView)
        onYesNoDiagUpdate: popUpView.yesNoDiagUpdate(title, body);
        onYesNoDiagClose: {
            popUpView.yesNoDiagClose()
            settingsMainView.diagWindowClosed()
        }
        onTextEntryRequest: settingsMainView.userTextEntryReq(defval, ishidden)
        onLanguageChanged:{
    ...
    KvalUiLiveStreamManager {
        id: kvalUiLiveStreamManager
    ...
    KvalUiGuiUtils {
        ...
        onDisplayMsg: pyNotificationView.requestAlertDisplay(header, null, msg);
    ...
    KvalUiLiveTvManager{
        ...
        onNotifyAlert: {
            pyNotificationView.requestAlertDisplay( type,
                kvalUiConfigManager.retranslate + qsTr("Live Tv Notification"),
                msg);
        }
    }
    ...
    UI_AppsElement {
        ...
        onDisplayAppsMsgSig: pyNotificationView.requestAlertDisplay(header, null, body)
        onOkDiag: popUpView.okDiag(title, text)
...
//        onAppDisplayAlert:
    ...
    UI_SubsClientElement{
        ...
        onDisplayInternalSrvNotifSignal: {
            pyNotificationView.requestAlertDisplay(header, null, body)
        }
    ...
    UI_MediaBrowserElement{
        ...
        onDisplayProgress: {
            pyNotificationView.requestProgressDisplay(
                        kvalUiConfigManager.retranslate + qsTr("Media Browser"),
                        msg, progressVal);
        }
        onDisplayMsg: {
            pyNotificationView.requestAlertDisplay(header, null, msg);
        }
    }
    ...
    UI_VodElement{
        ...
        onDisplayNotifProgressSignal :
        {
            pyNotificationView.requestProgressDisplay(
                        ...
                        notifStr, progressValue);
        }
        onDisplayNotifSignal:
        {pyNotificationView.requestAlertDisplay(headerMsg, null, mainMsg)
        }
        onReadyItemInfo :
...
//                appsView.stopNotify()
    ...
    function onPlayBackCompleted()
    {
        ...
        else if(currentNavLevel === IPTV_Constants.ENUM_CATEGORIES_APPLICATIONS_LEVEL)
        {
            ...
            mediaBrowserMainView.playBackCompleted()
        }
    }
