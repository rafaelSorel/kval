...
#define LOG_SRC BITSTREAMCONVERTER
...
bool CBitstreamConverter::Open(enum AVCodecID codec, uint8_t *in_extradata, int in_extrasize, bool to_annexb)
{
  ...
  switch(m_codec)
  {
    case AV_CODEC_ID_H264:
      ...
      {
        LOG_ERROR(LOG_TAG, "CBitstreamConverter::Open avcC data too small or missing");
        return false;
      }
      ...
      if(m_to_annexb)
      {
        if ( in_extradata[0] == 1 )
        {
          LOG_INFO(LOG_TAG,  "CBitstreamConverter::Open bitstream to annexb init");
          ...
          LOG_INFO(LOG_TAG,  "CBitstreamConverter::Open Invalid avcC");
      }
      else
      {
        ...
        if ( in_extradata[0] != 1 )
        {
          ...
          {
            LOG_INFO(LOG_TAG,  "CBitstreamConverter::Open annexb to bitstream init");
          ...
          {
            LOG_DEBUG(LOG_TAG, "CBitstreamConverter::Open invalid avcC atom data");
            return false;
          }
        }
        else
        {
          ...
          {
            LOG_INFO(LOG_TAG,  "CBitstreamConverter::Open annexb to bitstream init 3 byte to 4 byte nal");
    ...
    case AV_CODEC_ID_HEVC:
      ...
      {
        LOG_ERROR(LOG_TAG, "CBitstreamConverter::Open hvcC data too small or missing");
        return false;
      }
      ...
      if(m_to_annexb)
      {
        ...
        */
        ...
        if (in_extradata[0] || in_extradata[1] || in_extradata[2] > 1)
        {
          ...
          LOG_INFO(LOG_TAG,  "CBitstreamConverter::Open Invalid hvcC");
      }
      else
      {
        ...
        if ( in_extradata[0] != 1 )
        {
          ...
          {
            LOG_DEBUG(LOG_TAG, "CBitstreamConverter::Open invalid hvcC atom data");
            return false;
          }
        }
        else
      ...
      if (m_to_annexb)
      {
        ...
        if (m_convert_bitstream)
        {
          ...
          else
          {
            m_convertSize = 0;
            m_convertBuffer = NULL;
            LOG_ERROR(LOG_TAG, "CBitstreamConverter::Convert: error converting.");
            return false;
          }
        }
        else
...
pps:
  ...
  if (!sps_seen)
      LOG_DEBUG(LOG_TAG, "SPS NALU missing or invalid. The resulting stream may not play");
  if (!pps_seen)
      LOG_DEBUG(LOG_TAG, "PPS NALU missing or invalid. The resulting stream may not play");
