...
bool COMXOverlayCodecText::Open(CAMLStreamInfo &hints)
{
...
int COMXOverlayCodecText::Decode(unsigned char* data, int size, double pts, double duration)
{
  if(m_pOverlay)
    delete m_pOverlay;
  ...
  while(p<end)
  {
    if(*p == '{')
    {
      ...
      override[p-start] = '\0';
      //LOG_INFO(LOG_TAG,  "Skipped formatting tag %s", override);
      free(override);
