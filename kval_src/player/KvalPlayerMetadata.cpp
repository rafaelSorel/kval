...
KvalPlayerMetaData::KvalPlayerMetaData(QObject *parent) :
    QObject(parent)
{
}

void KvalPlayerMetaData::setValuesFromStatistics(const QtAV::Statistics &st)
{
    m_metadata.clear();
    //setValue(Size, st.);
...
KvalPlayerMetaData::Key KvalPlayerMetaData::fromFFmpegName(const QString &name) const
{
    typedef struct {
        Key key;
        const char* name;
...
void KvalPlayerMetaData::setValue(Key k, const QVariant &v)
{
    if (value(k) == v)
        return;
    m_metadata[k] = v;
...
QVariant KvalPlayerMetaData::value(Key k, const QVariant &defaultValue) const
{
    return m_metadata.value(k, defaultValue);
}

QString KvalPlayerMetaData::name(Key k) const
{
