...
#ifndef KVAL_LOGGING_H
#define KVAL_LOGGING_H
...
#define ENABLE_LOG_CRITICAL
#else
// If none is selected desable logs.
...
#ifdef RASP_PI_TARGET
static uint32_t min_stp = 0xFFFFFFFFU;
//ARM Architecture
static inline uint32_t __attribute__((pure)) get_stack_pointer(void)
{
    uint32_t value;
    asm volatile
    (
    ...
    if ( value < min_stp ) { min_stp = value; }
    return value;
}
#endif

#define ARRAY_SIZE(a) (sizeof(a)/sizeof((a)[0]))
#define ZARRAY_SIZE(a) ((sizeof(a)/sizeof((a)[0]))-1)
#define CARRET "\r\n"
...
public:
   ...
   void prependHeader(std::string& s, unsigned int);
...
public:
    typedef struct _thread_properties
    {
        uint8_t color;
        uint8_t attrib;
        uint8_t threadId;
   ...
   static void getColorForThread(unsigned int threadId, uint8_t *);
   static LC_LogAttrib getAttribForThread(LC_LogLevel level);
   static inline unsigned int get_thread_Number(unsigned int);
};
...
/**
 * @brief render_printable_chars
 * @param os
 * @param buffer
 * @param bufsize
 * @return
 */
inline std::ostream& render_printable_chars(std::ostream& os, const char* buffer, size_t bufsize) {
    os << " | ";
 ...
 * @return
 */
inline std::ostream& hex_dump(const uint8_t* buffer, size_t bufsize, bool showPrintableChars = true)
{
    ...
    for (; i < bufsize; ++i)
    {
        if (i % 16 == 0)
        {
            ...
            {
                render_printable_chars(os, reinterpret_cast<const char*>(&buffer[i] - 16), 16);
            }
            os << std::endl;
            printBlank = false;
        }
    ...
    if (i % 16 != 0 && showPrintableChars)
    {
        for (size_t j = 0; j < 16 - (i % 16); ++j)
        {
            os << "   ";
        }
        render_printable_chars(os, reinterpret_cast<const char*>(&buffer[i] - (i % 16)), (i % 16));
    }
...
#define MAINSTR                 "main"
#define UTILS                   "utils"
#define AMLMEDIAPLAYER          "aml_mp"
#define AMLMEDIAPLAYERELEMT     "AMLReader"
#define AMLVIDEOSINK            "AmlVideoSink"
#define AMLAUDIOCDCELEMT        "AudioCodec"
#define ALSAAUDIOSINK           "AlsaAudioSink"
#define AMLSUBS                 "SubtitleCodecElement"
#define AMLCODEC                "AmlCodec"
#define AMLCOMPONENT            "AMLComponent"
#define AMLEXCEPT               "aml_exception"
#define AMLIMAGE                "aml_image"
#define AMLINPIN                "aml_in_pin"
#define AMLOUTPIN               "aml_out_pin"
#define AMLTHREAD               "aml_thread"
#define BITSTREAMCONVERTER      "bitstrmconverter"



#define OMXWRAPPERSTR           "omx_wrapper"
#define OMXLIBSTR               "omx_lib"
#define OMXMEDIAPROCSTR         "omx_media_proc"
#define OMXMEDIAELEMENTSTR      "omx_media_element"
#define OMXTEXPROVIDERSTR       "omx_texture_provider"
#define OMXVIDEOSURFACESTR      "omx_video_surface"
#define MODELSTR                "model"
#define SOPENGINESTR            "sop_cast_engine"
#define EPGENGINESTR            "epg_engine"
#define RTMPENGINESTR           "rtmp_engine"
#define HTTPENGINESTR           "http_engine"
#define MEDIAPVRSTR             "media_pvr"
#define VODENGINESTR            "vod_engine"
#define SERVERNOTIFENGINESTR    "server_notif_engine"
#define APPSMANAGER             "appsManager"
#define EMBEDFUNCS              "embedFuncs"
#define MEDIA_BROWSER           "media_browser"
#define SETMANAGER              "settingsManager"
#define NETUTILS                "netUtils"
#define DOWNLOADER              "downloader"
#define SECURE                  "secure"
#define CONFIGMGR               "cfgmanager"
#define AMLPLATFORM             "AMLPlatform"
#define PLAYERHELPER            "PlayerHelper"
#define KVAL_LIVESTREAM_MANAGER "KvalLiveStreamManager"
#define KVAL_LIVETV_MANAGER     "KvalLiveTvManager"

#ifndef LOG_SRC
    #warning "Please define LOG_SRC before using logging system"
#endif // LOG_SRC
...
#define LOG_CRITICAL(tag, f, ...) \
   log_critical_t(tag, "%s::%s() [%d] " f, LOG_SRC, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define LOG_ERROR(tag, f, ...) \
   log_err_t(tag, "%s::%s() [%d] " f, LOG_SRC,__FUNCTION__, __LINE__, ##__VA_ARGS__)
#define LOG_WARNING(tag, f, ...) \
   log_warn_t(tag, "%s::%s() [%d] " f, LOG_SRC,__FUNCTION__, __LINE__, ##__VA_ARGS__)
#define LOG_INFO(tag, f, ...) \
   log_info_t(tag, "%s::%s() [%d] " f, LOG_SRC ,__FUNCTION__, __LINE__, ##__VA_ARGS__)
#define LOG_VERBOSE(tag, f, ...) \
   log_verbose_t(tag, "%s::%s() [%d] " f, LOG_SRC,__FUNCTION__, __LINE__, ##__VA_ARGS__)
#define LOG_DEBUG(tag, f, ...) \
   log_debug_t(tag, "%s::%s() [%d] " f, LOG_SRC,__FUNCTION__, __LINE__, ##__VA_ARGS__)
#define INV(f, ...) \
    log_debug_t(LOG_TAG, "> %s::%s() [%d] " f, LOG_SRC,__FUNCTION__, __LINE__, ##__VA_ARGS__)
#define OUTV(f, ...) \
    log_debug_t(LOG_TAG,"< %s::%s() [%d] " f, LOG_SRC,__FUNCTION__, __LINE__, ##__VA_ARGS__)
#define DUMP_HEX(_lvl_, _buf_, _len_) \
    if ( is_level_activated(_lvl_) ) \
        { hex_dump(_buf_, _len_) ; }
#else
#define LOG_CRITICAL(tag, f, ...)
#define LOG_ERROR(tag, f, ...)
#define LOG_WARNING(tag, f, ...)
...
/**
 * @brief is_level_activated
 * @param _lvl_
 * @return
 */
inline bool is_level_activated(LC_LogLevel _lvl_)
{
    LC_LogLevel active_level = LC_LOG_NONE;
 #ifdef BUILD_LOG_LEVEL_DEBUG
    active_level = LC_LOG_DEBUG;
 #elif defined(BUILD_LOG_LEVEL_VERBOSE)
    active_level = LC_LOG_VERBOSE;
 #elif defined(BUILD_LOG_LEVEL_INFORMATION)
    active_level = LC_LOG_INFO;
 #elif defined(BUILD_LOG_LEVEL_WARNING)
    active_level = LC_LOG_WARN;
 #elif defined(BUILD_LOG_LEVEL_ERROR)
    active_level = LC_LOG_ERROR;
 #elif defined(BUILD_LOG_LEVEL_CRITICAL)
    active_level = LC_LOG_CRITICAL;
 #endif

    if(_lvl_ >= active_level) return true;
    return false;
}
...
{
    char threadIdStr[3];
    sprintf(threadIdStr, "%02d", threadId);
    threadIdStr[2] = 0;

   s.append(lc_current_time() + " ");
   s.append("#" + (std::string)threadIdStr + " ");
   if (m_level != LC_LOG_NONE)
      s.append("["+toString(m_level)+"]"+ " ");
}
...
/**
 * @brief LC_Output2Std::get_thread_Number
 * @param thread_id
 * @return 
 */
inline unsigned int LC_Output2Std::get_thread_Number(unsigned int 
                                                    thread_id)
{
    static QHash<unsigned int, uint8_t> thread_list;
    static uint8_t thread_number = 0;
    if(thread_list.keys().contains(thread_id)) 
        return thread_list.value(thread_id);

    thread_number++;
    thread_list[thread_id] = thread_number;
...
inline void LC_Output2Std::printf(LC_Log<LC_Output2Std>& logger, va_list args)
{
   ...
   LC_LogColor  color;
#if __ARCH_64_BIT__
   QString threadId = QString::number((long)QThread::currentThreadId(), 16);
#else
   unsigned int threadNbr = get_thread_Number(
               (unsigned long)QThread::currentThreadId());
   uint8_t thread_prop[2];
#endif

#ifdef RASP_PI_TARGET
   uint32_t stp = get_stack_pointer();
   char stackPointer[10];
   sprintf(stackPointer, "0x%08x", stp);
#endif
   if (logger.m_level != LC_LOG_NONE) 
   {
        getColorForThread(threadNbr, thread_prop);
        color = (LC_LogColor)thread_prop[0];
        attrib = (LC_LogAttrib)thread_prop[1];
   }
   else
   {
   ...
   std::string input;
   logger.prependHeader(input, threadNbr);
...
|    LC_Output2Std::getColorForLevel
+-----------------------------------------------------------------------------*/
inline LC_LogAttrib LC_Output2Std::getAttribForThread(LC_LogLevel level)
{
    static const LC_LogAttrib LC_ATTRIB_MAP [] {
       LC_LOG_ATTR_BLINK,
       LC_LOG_ATTR_BLINK,
       LC_LOG_ATTR_UNDERLINE,
       LC_LOG_ATTR_DIM,
       LC_LOG_ATTR_DIM,
       LC_LOG_ATTR_DIM,
       LC_LOG_ATTR_DIM
   ...
   return LC_ATTRIB_MAP[level];
}

/*------------------------------------------------------------------------------
|    LC_Output2Std::getColorForThread
+-----------------------------------------------------------------------------*/
inline void LC_Output2Std::getColorForThread(unsigned int threadId, 
                                             uint8_t * thread_props)
{
    static const LC_LogColor LC_COLOR_MAP [] {
        LC_LOG_COL_RED,
        LC_LOG_COL_GREEN,
    ...
    static uint8_t fillColorTags = 0x0;
    static uint8_t i = 0;
    static uint8_t j = 0;
    static uint8_t k = 0;

    static thread_properties_t thread_list_prop[28];
    if(!fillColorTags)
    {
        fillColorTags = 0x1;
        for(i = 1; i<29; i++)
        {
            thread_list_prop[i-1].threadId = i;
            if( i < 8)
            {
                thread_list_prop[i-1].color = LC_COLOR_MAP[i-1];
                thread_list_prop[i-1].attrib = LC_LOG_ATTR_BRIGHT;
            }
            else if(i > 8 && i < 16)
            {
                thread_list_prop[i-1].color = LC_COLOR_MAP[j++];
                thread_list_prop[i-1].attrib = LC_LOG_ATTR_DIM;
            }
            else
            {
                thread_list_prop[i-1].color = LC_COLOR_MAP[k++];
                thread_list_prop[i-1].attrib = LC_LOG_ATTR_RESET;
            }
        }
    }


    thread_props[0] = thread_list_prop[threadId].color;
    thread_props[1] = thread_list_prop[threadId].attrib;
}
